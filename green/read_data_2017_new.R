#Goal - making changes to some functions
#get rid of fish per hour
#filter out rows with comments containing "stopper" and "block"
...
unique(df_2017$WSPEName)

#trying out if it works for one species
trial <- data_reformat("Chum", "0+", "M", 2017, "Green", ".accdb")
...
data_reformat <- function(species, age, origin, year, river, file_type){
  ...
  df_filter <- df[!((str_detect(df_2017$Comments, "stopper") | 
                       str_detect(df_2017$Comments, "block")) & 
                      !is.na(df_2017$Comments)),]
...
write_csv_2017 <- function(year, river, file_type){
  all_species = c("Chinook", "Coho", "Steelhead", "Chum")
  ...
  colnames(df_merge) <- c("start_datetime", "end_datetime", "In", "midpoint", "StartDate", "EndDate",
                          "chum0_mixed_num", "steelheadsmolt_wild_num",
                          "steelheadsmolt_hatchery_num","coho1_wild_num", "coho1_hatchery_num", 
                          "coho0_wild_num", "chinook1_hatchery_num",  "chinook0_wild_num",
...
trial <- write_csv_2017(2017, "Green", ".accdb")
