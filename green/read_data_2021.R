#Goal - copy code from 2020 to read 2021 data
...
df_2021 <- access_file("2021 Green.accdb")
unique(df_2021$WSPEName)
...
trial <- data_reformat("Steelhead", "smolt", "H", 2021, "Green", ".accdb")
#working but numbers are small except for chum
...
write_csv <- function(year, river, file_type){
  ...
  colnames(df_merge) <- c("start_datetime", "end_datetime", "In", "midpoint", "StartDate", "EndDate",
                          ...
                          "chinook1_hatchery_num",  "chinook0_wild_num",
...
trial <- write_csv(2021, "Green", ".accdb")
