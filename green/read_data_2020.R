#Goal - copy code from 2017 to read 2020 data
...
df_2020 <- access_file("2020 Green.accdb")
unique(df_2020$WSPEName)
...
data_reformat <- function(species, age, origin, year, river, file_type){
  ...
  df_filter <- df[!((str_detect(df$Comments, "stopper") | 
                       str_detect(df$Comments, "block")) & 
                      !is.na(df$Comments)),]
...
trial <- data_reformat("Coho", "1+", "M", 2020, "Green", ".accdb")
#working but numbers are small except for chum and pink
...
trial <- write_csv(2020, "Green", ".accdb")
