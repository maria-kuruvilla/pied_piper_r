#Goal - to write the newer MS Access files into csv so that I can 
...
#extract_species_data_error_csv <- function(year, species,type,river,file_type){
  ...
  df_species_all_years$In <- as.numeric(difftime(df_species_all_years$end_datetime,
...
write_csv <- function(year, river, file_type){
  all_species = c("Chinook", "Coho", "Steelhead")
  all_origin = c("H","W")
  count = 0 
  ...
  file_name = paste0(year,"_",tolower(river),"_all_R.csv")
  print(file_name)
